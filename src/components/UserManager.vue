...
methods: {
  ...
  filteredUsers() {
    return this.users.filter(
        user => 
            user.lastname.toLowerCase().indexOf(this.searchText.toLowerCase()) != -1 ||
            user.firstname.toLowerCase().indexOf(this.searchText.toLowerCase()) != -1
            );
  }
},
mounted () {
  axios.get('http://localhost:3000/users').then(response => (this.users = response.data))
}
}
</script>
